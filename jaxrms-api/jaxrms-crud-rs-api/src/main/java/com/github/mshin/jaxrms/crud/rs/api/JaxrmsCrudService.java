...
public interface JaxrmsCrudService {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public ResponseMessage putName(@PathParam("id") String id, PutNameRequest putNameRequest) throws ExceptionResponses;

    @GET
    @Path("/get")
    @ApiOperation(value = "This API method will be used to return a name record.")
    @Produces(MediaType.APPLICATION_JSON)
    public ResponseMessage getName(@QueryParam("id") String id);

    @DELETE
    @Path("/delete/{id}")
    @ApiOperation(value = "This API method will be used to delete a name record.")
    @Produces(MediaType.APPLICATION_JSON)
    public ResponseMessage deleteName(@PathParam("id") String id);
